...
import { addConsumer, createProfile, deleteProfile, getConsumers, healProfile, listProfiles, removeConsumer, seedPluginIntoProfile } from "./src/profileManager";
import {
	ApplyPlan,
	detachSingleItem,
	detectActiveProfile,
...
export default class ProfileSettingsPlugin extends Plugin {
	...
	/** 현재 vault 에 설치된 우리 플러그인 폴더의 절대경로. */
...
class ProfileSettingsTab extends PluginSettingTab {
	...
	private async renderAsync(): Promise<void> {
		...
		const folderSaveBtn = folderRow.createDiv({
	...
	private openCreateModal(): void {
		...
		new CreateProfileModal(this.app, async (name) => {
			try {
				...
				// 새 프로파일에 우리 플러그인을 기본 설치 — Activate 직후 빈 plugins
				// junction 으로 플러그인이 사라지는 혼동을 방지. best-effort.
				const pluginDir = this.plugin.getPluginDir();
				if (pluginDir) {
					try {
						await seedPluginIntoProfile(
							this.plugin.settings.profilesRoot,
							name,
							pluginDir,
						...
						console.error("[sync-settings] failed to seed plugin into new profile", err);
					}
				}
				await this.refreshProfiles();
				this.display();
	...
	private async handleActivate(name: string): Promise<void> {
		...
		// 이 기능 이전에 만든 프로파일은 plugins 에 우리 플러그인이 없어 Activate
		// 직후 사라질 수 있다. 없을 때만 채워넣는다(있으면 덮어쓰지 않음). best-effort.
		const selfDir = this.plugin.getPluginDir();
		if (selfDir) {
			try {
				await seedPluginIntoProfile(profilesRoot, name, selfDir, this.plugin.manifest.id, true);
			} catch (err) {
				console.error("[sync-settings] failed to seed plugin into existing profile", err);
			}
		}
