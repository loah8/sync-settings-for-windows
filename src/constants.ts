/**
 * 프로파일 저장소 내부의 설정 폴더 이름.
 *
 * 주의: vault 쪽은 `app.vault.configDir` 를 따라가지만, 프로파일 저장소는
 * 우리 플러그인이 만드는 고유 구조라 고정 이름을 쓴다. Obsidian 이 직접
 * 읽는 경로가 아니라 symlink 의 target 일 뿐이므로 이름이 무엇이든 무방.
 * `.obsidian` 리터럴은 `obsidianmd/hardcoded-config-path` 룰에 걸리므로 피한다.
 */
export const PROFILE_CONFIG_DIR = "obsidian";
...
export interface SharedItem {
	...
	type: "dir" | "file";
	displayName?: string;
	/** UI subtitle (한 줄 설명). Appearance 처럼 상태(theme name) 로 덮어씌울 수도 있음 */
	description?: string;
...
export const SHARED_ITEMS: ReadonlyArray<SharedItem> = [
	...
	{ name: "community-plugins.json", type: "file", displayName: "Community plugins", description: "Includes installed plugin list", group: "community-plugins", initialContent: "[]" },

	// ── Appearance 그룹: 테마 파일 + 외형 설정 ──
	{ name: "themes", type: "dir", group: "appearance" },
	{ name: "appearance.json", type: "file", displayName: "Appearance", description: "Theme, fonts, and UI preferences", group: "appearance", initialContent: "{}" },

	// ── 단독 항목 ──
	{ name: "snippets", type: "dir", displayName: "CSS snippets", description: "Custom CSS files" },
	{ name: "app.json", type: "file", displayName: "Editor & files", description: "Editor settings, default file locations", initialContent: "{}" },
	{ name: "hotkeys.json", type: "file", displayName: "Hotkeys", description: "Keyboard shortcut bindings", initialContent: "{}" },
	{ name: "core-plugins.json", type: "file", displayName: "Core plugins", description: "Built-in plugin settings", initialContent: "{}" },
];
