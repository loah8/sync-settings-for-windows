...
/**
 * 새 프로파일에 기본으로 함께 설치할 우리 플러그인의 릴리즈 파일들.
 * `styles.css` 는 없을 수도 있으므로(테마/CSS 없는 빌드) 누락을 허용한다.
 */
const SELF_PLUGIN_FILES: ReadonlyArray<{ name: string; optional?: boolean }> = [
	{ name: "main.js" },
	{ name: "manifest.json" },
	{ name: "styles.css", optional: true },
];

/**
 * 새로 만든 프로파일 안에 우리 플러그인 자신을 기본 설치한다.
 *
 * 이유: 프로파일을 Activate 하면 vault 의 `plugins/` 가 프로파일 저장소의
 * `plugins/` 를 가리키는 junction 으로 바뀐다. 갓 만든 프로파일의 `plugins/`
 * 가 비어 있으면 Activate 직후 우리 플러그인을 포함한 모든 플러그인이 사라져
 * 사용자가 프로파일 안에서 다시 설치해야 하는 혼동이 생긴다. 생성 시점에
 * 현재 설치된 플러그인 파일을 복사해 두면 Activate 후에도 살아남는다.
 *
 * - `sourcePluginDir`: 현재 vault 에 설치된 우리 플러그인 폴더 (절대경로)
 * - 항상 *현재 설치된 파일* 을 복사하므로 버전이 올라가도 새 프로파일은 최신
 * - `community-plugins.json` 에 `pluginId` 를 추가해 Activate 후 활성 상태로 시작
 *
 * `onlyIfMissing: true` 면 프로파일에 우리 플러그인이 이미 있으면(= `main.js`
 * 존재) 아무것도 하지 않는다. 기존 설치물/다른 vault 것을 덮어쓰지 않으면서,
 * 이 기능 이전에 만들어진 프로파일(플러그인 없음)만 채워넣을 때 쓴다.
 *
 * - 생성(`createProfile`) 직후: `onlyIfMissing` 없이 호출(항상 최신으로 심음)
 * - 기존 프로파일 Activate 직전: `onlyIfMissing: true` 로 호출(없을 때만 보충)
 */
export async function seedPluginIntoProfile(
	profilesRoot: string,
	profileName: string,
	...
	if (!profilesRoot || !profileName || !sourcePluginDir || !pluginId) return;

	const obsidianDir = path.join(profilesRoot, profileName, PROFILE_CONFIG_DIR);
	const destPluginDir = path.join(obsidianDir, "plugins", pluginId);

	// 이미 설치돼 있으면(main.js 존재) 건드리지 않는다.
	if (onlyIfMissing && (await pathExists(path.join(destPluginDir, "main.js")))) {
		return;
	}

	await fs.mkdir(destPluginDir, { recursive: true });

	for (const file of SELF_PLUGIN_FILES) {
		const src = path.join(sourcePluginDir, file.name);
		const dest = path.join(destPluginDir, file.name);
		try {
			await fs.copyFile(src, dest);
		} catch (err) {
			const e = err as NodeJS.ErrnoException;
			if (e.code === "ENOENT" && file.optional) continue;
			throw err;
		}
	}

	// community-plugins.json (활성 플러그인 목록) 에 우리 id 를 추가
	...
	try {
		...
		if (Array.isArray(parsed)) {
			enabled = parsed.filter((x): x is string => typeof x === "string");
		}
	} catch {
		// 없거나 깨졌으면 새로 만든다
	}
	if (!enabled.includes(pluginId)) {
		...
		await fs.writeFile(cpPath, JSON.stringify(enabled, null, 2), "utf8");
	}
}
