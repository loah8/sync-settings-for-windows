...
import { LOCAL_ONLY_FILES, SharedItem, SHARED_ITEMS } from "./constants";
...
export async function canCreateSymlink(): Promise<boolean> {
	...
	const targetPath = path.join(tmp, `obsidian-profile-settings-probe-target-${stamp}.tmp`);
	const linkPath = path.join(tmp, `obsidian-profile-settings-probe-link-${stamp}.tmp`);
 ...
 *
 * 진실의 원천: vault `.obsidian/plugins`. symlink/junction 의 target 이
 * `<profilesRoot>/<X>/.obsidian/plugins` 형태면 X 가 활성.
 */
export async function detectActiveProfile(
	...
	const linkPath = path.join(vaultPath, ".obsidian", "plugins");
	...
	if (parts[1] !== ".obsidian" || parts[2] !== "plugins") return null;
...
export async function planApply(
	...
	const profileObsidian = path.join(profilesRoot, profileName, ".obsidian");
	const vaultObsidian = path.join(vaultPath, ".obsidian");
 ...
 */
export async function executeUnlink(vaultPath: string): Promise<string[]> {
	const vaultObsidian = path.join(vaultPath, ".obsidian");
...
export async function unlinkSingleItem(
	...
	const linkPath = path.join(vaultPath, ".obsidian", item.name);
...
export async function executeApply(vaultPath: string, plan: ApplyPlan): Promise<void> {
	const vaultObsidian = path.join(vaultPath, ".obsidian");
