...
When you create a new profile, this plugin installs itself into it automatically, so it keeps working right after you apply the profile.
...
- **Changes apply on next open.** Obsidian reads `.obsidian` settings when a vault opens. When you change settings in a vault using an active profile, they are saved to the profile right away — but other vaults already open with the same profile will not reflect the change until they are restarted.
